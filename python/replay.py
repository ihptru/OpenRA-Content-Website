...
print("uploaded file")
Data = open(replay_path).read(600000)
...
sql = """INSERT INTO replays
        (title,path,user_id,date_time,duration,server_name,maphash,mods)
        VALUES
        (
        '%(title)s',
        '%(path)s',
...
# last clients
clients = Data.split('GlobalSettings')[-2]
...
for i in range(len(client_names)):
    sql = """INSERT INTO replay_players
            (id_replays,name,colorramp,country,team,winner)
            VALUES
            (
            %s,
            '%s',
