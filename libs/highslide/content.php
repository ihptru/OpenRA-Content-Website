...
class content
{
	...
	while ($comment = db::nextRowFromQuery($result))
	{
	    ...
	    $avatarImg = misc::avatar($author["avatar"], $author["login"]);
	...
	*/
	...
	elseif ($page == "members")
	{
		...
		while ($row = db::nextRowFromQuery($result))
		{
		    $avatar = misc::avatar($row["avatar"], $row["login"]);
...
class profile
{
    public static function show_profile()
    {
		...
		elseif ($avatar == "done")
		{
		    echo "<img src='users/".user::username()."/avatar.jpg'>";
		}
    ...
    {
	$query = "SELECT avatar,login FROM users WHERE uid = " . user::uid();
	$result = db::executeQuery($query);
	$row = db::nextRowFromQuery($result)
	$avatar = misc::avatar($row["avatar"], $row["login"]);
	echo "<img src='".$avatar."' width='120px'>";
    }
